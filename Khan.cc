...
// THIS IS THE BASELINE SIMULATION CODE
...
// by M. A. Khan, et al. (DOI: 10.1109/TVT.2021.3055895)
...
// --- Communication Cost (Khan et al., Table V) ---
// Total: 1280 bits = 160 bytes, distributed among 3 messages
#define BASELINE_PSI1_SIZE 60
#define BASELINE_PSI2_SIZE 80
#define BASELINE_PSI3_SIZE 20

#define NUAV 20         // Edit this!!!
#define ENDTIME 1500


#define BASELINE_PHASE1_DELAY 0.4931  // Initiator prepares Psi1
#define BASELINE_PHASE2_DELAY 3.42145  // Responder prepares Psi2
#define BASELINE_PHASE3_DELAY 2.92945  // Initiator prepares Psi3

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("UAV_AKA_Baseline_Khan_Simulation");
...
//----------------------------------------------------------------------
//-- UAVApp: Implements the 3-way handshake protocol from Khan et al.
//----------------------------------------------------------------------
...
private:
  ...
  void SendPsi1 (uint16_t targetId);
  void HandlePsi1_SendPsi2 (Ptr<Packet> packet, uint16_t sourceId);
  void HandlePsi2_SendPsi3 (Ptr<Packet> packet, uint16_t sourceId);
  void HandlePsi3_Finalize (Ptr<Packet> packet, uint16_t sourceId);

  void SocketRecv (Ptr<Socket> socket);

  void DoSendPsi1();
  void DoSendPsi2(uint16_t targetId);
  void DoSendPsi3(uint16_t targetId);
...
void UAVApp::StartAuthentication()
{
    ...
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " ---> starts Khan AKA with UAV " << targetId);
    SendPsi1(targetId);
}

void UAVApp::SendPsi1 (uint16_t targetId)
{
  Simulator::Schedule(MilliSeconds(BASELINE_PHASE1_DELAY), &UAVApp::DoSendPsi1, this);
}

void UAVApp::DoSendPsi1()
{
  Ptr<Packet> packet = Create<Packet> (BASELINE_PSI1_SIZE);
  MyHeader header;
  ...
  NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " --Ψ1--> sends to UAV " << m_session_target_id);
}
...
void UAVApp::SocketRecv (Ptr<Socket> socket)
{
  ...
  if (packet) {
    // NEW: Accumulate the size of the received packet
    ...
    switch (header.GetMsgType()) {
      case 1: HandlePsi1_SendPsi2(packet, header.GetSourceId()); break;
      case 2: HandlePsi2_SendPsi3(packet, header.GetSourceId()); break;
      case 3: HandlePsi3_Finalize(packet, header.GetSourceId()); break;
      default: break;
    }
  }
}

void UAVApp::HandlePsi1_SendPsi2 (Ptr<Packet> packet, uint16_t sourceId)
{
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " <--Ψ1-- received from UAV " << sourceId);
    Simulator::Schedule(MilliSeconds(BASELINE_PHASE2_DELAY), &UAVApp::DoSendPsi2, this, sourceId);
}

void UAVApp::DoSendPsi2(uint16_t targetId)
{
    Ptr<Packet> packet = Create<Packet> (BASELINE_PSI2_SIZE);
    MyHeader header;
    ...
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " --Ψ2--> sends back to UAV " << targetId);
}

void UAVApp::HandlePsi2_SendPsi3 (Ptr<Packet> packet, uint16_t sourceId)
{
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " <--Ψ2-- received from UAV " << sourceId);

    if (!m_is_authenticating || sourceId != m_session_target_id) {
        NS_LOG_WARN("Received unexpected Psi2 from " << sourceId);
        return;
    }

    FinalizeAuthentication(true);
    Simulator::Schedule(MilliSeconds(BASELINE_PHASE3_DELAY), &UAVApp::DoSendPsi3, this, sourceId);
}

void UAVApp::DoSendPsi3(uint16_t targetId)
{
    Ptr<Packet> packet = Create<Packet> (BASELINE_PSI3_SIZE);
    MyHeader header;
    ...
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " --Ψ3--> sends to UAV " << targetId);
}

void UAVApp::HandlePsi3_Finalize (Ptr<Packet> packet, uint16_t sourceId)
{
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " <--Ψ3-- received from UAV " << sourceId << ". Khan AKA complete.");
}
...
void UAVApp::FinalizeAuthentication(bool success)
{
    if (success) {
        ...
        NS_LOG_UNCOND("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " <---> KHAN AKA SUCCESS with UAV " << m_session_target_id << ". Total Delay: " << delay.GetMilliSeconds() << " ms");
    } else {
        NS_LOG_UNCOND("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " <---> KHAN AKA FAILED with UAV " << m_session_target_id);
    }
...
int main (int argc, char *argv[])
{
  ...
  AnimationInterface anim ("uav-aka-baseline-khan-animation.xml");
  for (uint32_t j = 0; j < NUAV; ++j) {
      std::ostringstream nodeDesc;
      nodeDesc << "UAV-" << j;
      anim.UpdateNodeDescription (uavNodes.Get(j), nodeDesc.str());
      anim.UpdateNodeColor (uavNodes.Get(j), 255, 0, 0); // Set baseline to red
  }
  
  Simulator::Stop (Seconds (ENDTIME+2));
  NS_LOG_UNCOND("Starting simulation for Baseline (Khan et al.) for " << ENDTIME << " seconds...");
  Simulator::Run ();
  ...
  for(uint16_t i = 0; i < NUAV; i++) {
    ...
    // NEW: Aggregate bytes from each app
    totalBytesReceived += uavApps[i]->GetTotalBytesReceived();
  }
  
  std::cout << "\n----------------------------------------------------" << std::endl;
  std::cout << "---      Simulation Final Results (Baseline)     ---" << std::endl;
  std::cout << "----------------------------------------------------" << std::endl;
