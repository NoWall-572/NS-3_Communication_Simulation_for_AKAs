...
// THIS IS THE FOURTH BASELINE SIMULATION CODE
// This code implements the AKA protocol from the paper:
// "Private blockchain-based access control mechanism for unauthorized UAV detection and mitigation in Internet of Drones environment" (ACSUD-IoD)
// by B. Bera, A. K. Das, and A. K. Sutrala (DOI: 10.1016/j.comcom.2020.12.005)
...
// --- Communication Cost (Bera et al., Table 7, Case 1) ---
#define BASELINE4_MSG1_SIZE 108 // 864 bits
#define BASELINE4_MSG2_SIZE 152 // 1216 bits
#define BASELINE4_MSG3_SIZE 36  // 288 bits

#define NUAV 20         // Edit this!!!
#define ENDTIME 1500    // 

// --- Computational Cost (Bera et al., Table 6, Case 1) ---
#define BASELINE4_DRONE_PREP_MSG1_DELAY 0.33  // Drone prepares Msg1 (1 hash + 1 senc)
#define BASELINE4_GSS_PREP_MSG2_DELAY   1.851 // GSS prepares Msg2 (total server cost)
#define BASELINE4_DRONE_PREP_MSG3_DELAY 7.405 // Drone prepares Msg3 (total drone cost)

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("UAV_AKA_Baseline_Bera_Simulation");
...
//----------------------------------------------------------------------
//-- UAVApp: Implements the 3-way handshake ACSUD-IoD protocol from Bera et al.
//----------------------------------------------------------------------
...
void UAVApp::StartAuthentication()
{
    ...
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " ---> starts Bera/ACSUD-IoD with UAV " << targetId);
    SendMsg1(targetId);
}

void UAVApp::SendMsg1 (uint16_t targetId)
{
  Simulator::Schedule(MilliSeconds(BASELINE4_DRONE_PREP_MSG1_DELAY), &UAVApp::DoSendMsg1, this);
}

void UAVApp::DoSendMsg1()
{
  Ptr<Packet> packet = Create<Packet> (BASELINE4_MSG1_SIZE);
  MyHeader header;
...
void UAVApp::HandleMsg1_SendMsg2 (Ptr<Packet> packet, uint16_t sourceId)
{
    ...
    Simulator::Schedule(MilliSeconds(BASELINE4_GSS_PREP_MSG2_DELAY), &UAVApp::DoSendMsg2, this, sourceId);
}

void UAVApp::DoSendMsg2(uint16_t targetId)
{
    Ptr<Packet> packet = Create<Packet> (BASELINE4_MSG2_SIZE);
    MyHeader header;
...
void UAVApp::HandleMsg2_SendMsg3 (Ptr<Packet> packet, uint16_t sourceId)
{
    ...
    Simulator::Schedule(MilliSeconds(BASELINE4_DRONE_PREP_MSG3_DELAY), &UAVApp::FinalizeInitiatorSuccess, this);
}

void UAVApp::DoSendMsg3(uint16_t targetId)
{
    Ptr<Packet> packet = Create<Packet> (BASELINE4_MSG3_SIZE);
    MyHeader header;
...
{
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " <--Msg3-- received from UAV " << sourceId << ". ACSUD-IoD complete for responder.");
}

void UAVApp::FinalizeInitiatorSuccess()
{
    m_auth_success++;
    Time delay = (Simulator::Now() - m_auth_start_time) + MilliSeconds(BASELINE4_DRONE_PREP_MSG3_DELAY);
    m_total_auth_delay += delay;
    NS_LOG_UNCOND("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " <---> BERA/ACSUD-IoD SUCCESS with UAV " << m_session_target_id << ". Total Delay: " << delay.GetMilliSeconds() << " ms");
...
int main (int argc, char *argv[])
{
  ...
  AnimationInterface anim ("uav-aka-baseline-bera-animation.xml");
  for (uint32_t j = 0; j < NUAV; ++j) {
      std::ostringstream nodeDesc;
      nodeDesc << "UAV-" << j;
      anim.UpdateNodeDescription (uavNodes.Get(j), nodeDesc.str());
      anim.UpdateNodeColor (uavNodes.Get(j), 128, 0, 128); // Set baseline to purple
  }
  
  Simulator::Stop (Seconds (ENDTIME+2));
  NS_LOG_UNCOND("Starting simulation for Baseline (Bera et al.) for " << ENDTIME << " seconds...");
  Simulator::Run ();
  ...
  std::cout << "\n----------------------------------------------------" << std::endl;
  std::cout << "---      Simulation Final Results (Baseline 4)   ---" << std::endl;
  std::cout << "----------------------------------------------------" << std::endl;
