...
// THIS IS THE THIRD BASELINE SIMULATION CODE
// This code implements the AKA protocol from the paper:
// "An Edge Assisted Secure Lightweight Authentication Technique for Safe Communication on the Internet of Drones Network" (SLPAKA)
// by M. Yahuza, et al. (DOI: 10.1109/ACCESS.2021.3060420)
...
// --- Communication Cost (Yahuza et al., Table 6) ---
#define BASELINE3_MSG1_SIZE 94
#define BASELINE3_MSG2_SIZE 94
#define BASELINE3_MSG3_SIZE 32

#define NUAV 20         // Edit this!!!
#define ENDTIME 1500

// --- Computational Cost (Yahuza et al., Table 5) ---
#define BASELINE3_INITIATOR_PREP_DELAY 2.23
#define BASELINE3_RESPONDER_PREP_DELAY 6.83
#define BASELINE3_INITIATOR_FINAL_DELAY 4.60

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("UAV_AKA_Baseline_Yahuza_Simulation");
...
//----------------------------------------------------------------------
//-- UAVApp: Implements the 3-way handshake SLPAKA protocol from Yahuza et al.
//----------------------------------------------------------------------
...
void UAVApp::StartAuthentication()
{
    ...
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " ---> starts Yahuza/SLPAKA with UAV " << targetId);
    SendMsg1(targetId);
}

void UAVApp::SendMsg1 (uint16_t targetId)
{
  Simulator::Schedule(MilliSeconds(BASELINE3_INITIATOR_PREP_DELAY), &UAVApp::DoSendMsg1, this);
}

void UAVApp::DoSendMsg1()
{
  Ptr<Packet> packet = Create<Packet> (BASELINE3_MSG1_SIZE);
  MyHeader header;
...
void UAVApp::HandleMsg1_SendMsg2 (Ptr<Packet> packet, uint16_t sourceId)
{
    ...
    Simulator::Schedule(MilliSeconds(BASELINE3_RESPONDER_PREP_DELAY), &UAVApp::DoSendMsg2, this, sourceId);
}

void UAVApp::DoSendMsg2(uint16_t targetId)
{
    Ptr<Packet> packet = Create<Packet> (BASELINE3_MSG2_SIZE);
    MyHeader header;
...
void UAVApp::HandleMsg2_SendMsg3 (Ptr<Packet> packet, uint16_t sourceId)
{
    ...
    Simulator::Schedule(MilliSeconds(BASELINE3_INITIATOR_FINAL_DELAY), &UAVApp::FinalizeInitiatorSuccess, this);
}

void UAVApp::DoSendMsg3(uint16_t targetId)
{
    Ptr<Packet> packet = Create<Packet> (BASELINE3_MSG3_SIZE);
    MyHeader header;
...
{
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " <--Msg3-- received from UAV " << sourceId << ". SLPAKA complete for responder.");
}

void UAVApp::FinalizeInitiatorSuccess()
{
    m_auth_success++;
    Time delay = (Simulator::Now() - m_auth_start_time) + MilliSeconds(BASELINE3_INITIATOR_FINAL_DELAY);
    m_total_auth_delay += delay;
    NS_LOG_UNCOND("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " <---> YAHUZA/SLPAKA SUCCESS with UAV " << m_session_target_id << ". Total Delay: " << delay.GetMilliSeconds() << " ms");
...
int main (int argc, char *argv[])
{
  ...
  AnimationInterface anim ("uav-aka-baseline-yahuza-animation.xml");
  for (uint32_t j = 0; j < NUAV; ++j) {
      std::ostringstream nodeDesc;
      nodeDesc << "UAV-" << j;
      anim.UpdateNodeDescription (uavNodes.Get(j), nodeDesc.str());
      anim.UpdateNodeColor (uavNodes.Get(j), 255, 255, 0); // Set baseline to yellow
  }
  
  Simulator::Stop (Seconds (ENDTIME+2));
  NS_LOG_UNCOND("Starting simulation for Baseline (Yahuza et al.) for " << ENDTIME << " seconds...");
  Simulator::Run ();
  ...
  std::cout << "\n----------------------------------------------------" << std::endl;
  std::cout << "---      Simulation Final Results (Baseline 3)   ---" << std::endl;
  std::cout << "----------------------------------------------------" << std::endl;
