...
// THIS IS THE SIXTH BASELINE SIMULATION CODE
...
// This file simulates the UAV-2-UAV scheme specifically.
...
// --- Communication Cost (Huang et al., Table V, UAV-2-UAV core) ---
#define BASELINE6_MSG1_SIZE 20  // 160 bits
#define BASELINE6_MSG2_SIZE 104 // 832 bits
#define BASELINE6_MSG3_SIZE 84  // 672 bits

#define NUAV 20         // Edit this!!!
#define ENDTIME 1500

// --- Computational Cost (Huang et al., Table IV, UAV-2-UAV) ---
#define BASELINE6_UAV_PREP_MSG2_DELAY 4.41
#define BASELINE6_UAV_PREP_MSG3_DELAY 4.4156

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("UAV_AKA_Baseline_Huang_U2U_Simulation");
...
//----------------------------------------------------------------------
//-- UAVApp: Implements the BAKAS-UAV (UAV-2-UAV) protocol
//----------------------------------------------------------------------
...
void UAVApp::StartAuthentication()
{
    ...
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " ---> starts Huang/BAKAS-UAV (U2U) with UAV " << targetId);
    SendMsg1(targetId);
}
...
{
  Ptr<Packet> packet = Create<Packet> (BASELINE6_MSG1_SIZE);
  MyHeader header;
...
void UAVApp::HandleMsg1_SendMsg2 (Ptr<Packet> packet, uint16_t sourceId)
{
    ...
    Simulator::Schedule(MilliSeconds(BASELINE6_UAV_PREP_MSG2_DELAY), &UAVApp::DoSendMsg2, this, sourceId);
}

void UAVApp::DoSendMsg2(uint16_t targetId)
{
    Ptr<Packet> packet = Create<Packet> (BASELINE6_MSG2_SIZE);
    MyHeader header;
...
void UAVApp::HandleMsg2_SendMsg3 (Ptr<Packet> packet, uint16_t sourceId)
{
    ...
    Simulator::Schedule(MilliSeconds(BASELINE6_UAV_PREP_MSG3_DELAY), &UAVApp::ComputeKeyAndFinalize, this);
}

void UAVApp::ComputeKeyAndFinalize()
{
    m_auth_success++;
    Time delay = Simulator::Now() - m_auth_start_time;
    m_total_auth_delay += delay;
    NS_LOG_UNCOND("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " <---> HUANG/BAKAS-UAV (U2U) SUCCESS with UAV " << m_session_target_id << ". Total Delay: " << delay.GetMilliSeconds() << " ms");
...
{
    Ptr<Packet> packet = Create<Packet> (BASELINE6_MSG3_SIZE);
    MyHeader header;
...
{
    NS_LOG_INFO("Time " << Simulator::Now().GetSeconds() << "s: UAV " << m_id << " <--Msg3-- received from UAV " << sourceId << ". BAKAS-UAV (U2U) complete for responder.");
}
...
int main (int argc, char *argv[])
{
  ...
  AnimationInterface anim ("uav-aka-baseline-huang-u2u-animation.xml");
  for (uint32_t j = 0; j < NUAV; ++j) {
      std::ostringstream nodeDesc;
      nodeDesc << "UAV-" << j;
      anim.UpdateNodeDescription (uavNodes.Get(j), nodeDesc.str());
      anim.UpdateNodeColor (uavNodes.Get(j), 255, 165, 0);
  }
  
  Simulator::Stop (Seconds (ENDTIME+2));
  NS_LOG_UNCOND("Starting simulation for Baseline (Huang et al., U2U) for " << ENDTIME << " seconds...");
  Simulator::Run ();
  ...
  std::cout << "\n----------------------------------------------------" << std::endl;
  std::cout << "---      Simulation Final Results (Baseline 6)   ---" << std::endl;
  std::cout << "----------------------------------------------------" << std::endl;
